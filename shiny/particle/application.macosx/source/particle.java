...
class Particle{
  ...
  public float sqd(float px, float py){
    return (x-px) * (x-px) + (y-py)*(y-py);
  }
  public void force(float fact, float px, float py){
    float f = sqd(px,py);
    vx += (x-px) * fact/f;
    vy += (y-py) * fact/f;
  }
...
//    stroke(255,255-v*50,255-v*100);
    ...
    if (positions[(int)x][(int)y] !=null){
      ...
      float rx = random(-0.5f,0.5f), ry = random(-0.5f,0.5f);
      vx += rx;
      vy += ry;
      p.vx -= rx;
      p.vy -= ry;
...
//    ellipse(x,y,10,10);
  ...
  Particle(float _x, float _y){
    this(_x,_y,random(-0.5f,0.5f),random(-0.5f,0.5f),0,3);
  }
}
Particle[] parts;
Particle[][] positions;
public void setup(){
  ...
  for (int i=0;i<parts.length;i++){
    if (random(1) < 0.5f){
      parts[i] = new Particle(random(1)<0.5f?5:width-5, random(height-5));
    }else{
      parts[i] = new Particle(random(width-5), random(1)<0.5f?5:height-5);
    }
  }
  lastframe = millis();
}
...
public void draw(){
  long now = millis();
  float delta = 0.001f * (now - lastframe);
  cx = width/2 + width/2*cos(-0.001f * now);
  cy = height/2 + height/2*sin(-0.001f * now);
  background(0);
  loadPixels();
...
/*
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#DFDFDF", "particle" });
  }
}
