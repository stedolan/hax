...
public void drawEdges(){
  int[] clrs = {color(255,0,0), color(0,255,0)};
  for (int i=0;i<edge_x.length;i++){
    fill(clrs[i%2]);
    ellipse(edge_x[i], edge_y[i], 10, 10);
  }
}
public void drawShadows(float x, float y){
  noStroke();
  fill(0);
  int i = 0;
...
public boolean ang_lt(float x1, float x2, float y1, float y2){
 return x1*y2 < y1*x2;
}

public void calcEdges(float x, float y){
  int i = 0, j = 0;
  while (i < triangle_x.length){
    float ax = triangle_x[i], bx = triangle_x[i+1], cx = triangle_x[i+2];
    float ay = triangle_y[i], by = triangle_y[i+1], cy = triangle_y[i+2];
...
//  drawEdges();
  ...
  static public void main(String args[]) {
    PApplet.main(new String[] { "--bgcolor=#DFDFDF", "shade" });
  }
}
