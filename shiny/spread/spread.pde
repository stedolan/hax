...
float spawnprob = 0.3;
int nheads=1;
float[] xheads={250},yheads={250},v={0.5};
int[] dirs={2};
color[] cols;

void reset(){
  nheads = 1 + int(random(3));
  ...
  v = new float[]{0.5,0.5,0.5};
  dirs = new int[]{(int)random(0,4), (int)random(0,4), (int)random(0,4)};
  cols = new color[]{color(0),color(0),color(0)};
  oldch = 10;
  decaying = false;
}
  ...
  for (int i=0;i<nheads;i++){
    ...
    if (oldp == p){
      ...
      if (random(1) < spawnprob){
        ...
        nyheads[nh] = yheads[i];
        ncols[nh] = cols[i];
        nv[nh] = constrain(v[i] * random(0.1, 1.1), 0.1, 1.0);
        nh++;
      }
      nxheads[nh]=xheads[i];
      nyheads[nh]=yheads[i];
